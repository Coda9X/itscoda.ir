document.addEventListener("DOMContentLoaded", () => {
    ...
    const HOME = (window as any).HOME_URL || document.querySelector<HTMLMetaElement>('meta[name="home-url"]')?.content || location.origin;

    // Handler: noscript to img/svg
    (() => {
        document.querySelectorAll("noscript[icon]").forEach(async (noscript) => {
            const container = noscript.parentElement as HTMLAnchorElement;
            if (!container) return;
    ...
    (() => {
        const typography = document.querySelector("#Enter p.typography") as HTMLElement | null;
        if (typography) {
            const typerAttr = typography.getAttribute("typer");
            if (!typerAttr) throw new Error("No 'typer' attribute found.");
            ...
            const spans = [...typography.querySelectorAll<HTMLSpanElement>("span")];
            ...
            (async function loop() {
                for (; ;) {
                    ...
                    const curIs = [...spans[cur].querySelectorAll<HTMLElement>("i")];
                    const nextIs = [...spans[next].querySelectorAll<HTMLElement>("i")];
                    ...
                    for (let k = 0; k < steps; k++) {
                        if (k < curIs.length) curIs[k].classList.remove("on");
                        if (k < nextIs.length) nextIs[k].classList.add("on");
                        await sleep(100);
                    }
                    cur = next;
                }
    ...
    (() => {
        ...
        const SPEED = 40, SPAWN = 1, SMIN = 2, SMAX = 12, IDLE = "#fff6", ACTIVE = "#fff6", MAX_IDLE = 20, DECAY_K = 1, MIN_V = 10, layers: L[] = [];
        type Star = { x: number; y: number; v: number; s: number };
        type L = {
            li: HTMLElement,
            c: HTMLCanvasElement,
        ...
        document.querySelectorAll<HTMLElement>(SEL).forEach(li => {
            li.style.position ||= "relative";
            const c = document.createElement("canvas"), g = c.getContext("2d")!;
            Object.assign(c.style, { position: "absolute", inset: "0", pointerEvents: "none", zIndex: "0" }); li.prepend(c);
            const L: L = { li, c, g, w: 1, h: 1, dpr: 1, stars: [], carry: 0, hover: false, dirty: true };
            const fit = () => {
                ...
                L.stars = [];
                seedIdle(L); L.dirty = true;
            };
            fit(); new ResizeObserver(fit).observe(li);

            li.addEventListener("mouseenter", () => {
                L.hover = true;
                L.dirty = true;
                for (const s of L.stars) if (s.v === 0) s.v = SPEED * (0.6 + Math.random() * 0.8)
            });
                ...
                for (const L of layers) {
                    ...
                    if (L.hover) {
                        ...
                        for (let i = L.stars.length - 1; i >= 0; i--) {
                            const s = L.stars[i];
                            s.x += s.v * dt;
                            if (s.x - s.s > L.w) L.stars.splice(i, 1)
                        }
                        ...
                        if (moving) {
                            ...
                            if (L.dirty) {
                                g.clearRect(0, 0, L.w, L.h); g.fillStyle = IDLE;
                                for (const s of L.stars) {
                                    g.font = `${s.s}px system-ui,Segoe UI,Roboto,sans-serif`;
                                    g.fillText("✦", s.x, s.y);
                                }
    ...
    (() => {
        ...
        const form = document.querySelector<HTMLFormElement>("#sayhi_form");

        const toObj = (fd: FormData) => {
            const o: Record<string, unknown> = {};
            fd.forEach((v, k) => o[k] = v instanceof File ? { name: v.name, size: v.size, type: v.type } : String(v));
            return o;
        };

        const save = (entry: unknown) => {
        ...
        form?.addEventListener("submit", async (event: Event): Promise<void> => {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    const msg = data?.errors?.map((e: { message: string }) => e.message).join(", ") || "Oops! There was a problem submitting your form";
                ...
                save({ ...base, status: "network_error", msg: (error as Error)?.message || "Network error" });
    ...
    // Handler: Play Song
    (() => {
        const btn = document.querySelector<HTMLElement>("#music");
        if (!btn) return;

        const src = new URL("assets/audio/Miriam.mp3", HOME).toString();
        let audio: HTMLAudioElement | null = null, playing = false;
        ...
        btn.addEventListener("click", async () => {
            ...
            try {
                ...
                alertify("warning", "Oops! something wrong to play music: " + (error as Error)?.message);
            }
        });
    })();
    ...
    (() => {
        const menu = document.querySelector<HTMLElement>("body > header > nav:nth-child(3)");
        const btn = document.querySelector<HTMLInputElement>("#Menu");

        if (!btn || !menu) return;

        const close = () => {
            btn.checked = false; btn.dispatchEvent(new Event('change', { bubbles: true }));
        };
        addEventListener("pointerdown", e => {
            const t = e.target as Element;
            if (btn.checked && !(menu.contains(t) || t === btn || t.closest?.("#Menu"))) close();
        }, true);
    ...
    function alertify(type: "success" | "info" | "warning" | "error", msg: string, time = 4000): void {
        ...
        el.addEventListener("mouseleave", () => { if (!closed) timer = window.setTimeout(remove, 1200); });
        el.addEventListener("click", remove);
    }

    // Global Function: Play video in console
    function consoleImage(url: string) {
